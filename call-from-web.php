 ...
 * Description: Let your visitors contact you by phone.
 * Version: 2.3
...
function cfw_options_init_fn(){
  register_setting('plugin_options', 'plugin_options', 'cfw_plugin_options_validate' );
  add_settings_section('main_section', 'Button Settings', 'cfw_section_text_fn', __FILE__);
  add_settings_field('cfw_profile', 'Widget ID', 'cfw_setting_profile', __FILE__, 'main_section');
  add_settings_field('cfw_button_title', 'Button Title', 'cfw_setting_button_title', __FILE__, 'main_section');
}
...
function cfw_setting_profile() {
  ...
  echo "<input id='plugin_text_string' name='plugin_options[cfw_profile]' size='40' type='text' value='{$options['cfw_profile']}' />";
}

function cfw_setting_key() {
  $options = get_option('plugin_options');
  echo "<input id='plugin_text_string' name='plugin_options[cfw_key]' size='40' type='text' value='{$options['cfw_key']}' />";
}

function cfw_setting_button_title() {
  $options = get_option('plugin_options');
  $button_title = empty($options['cfw_button_title']) ? 'Contact Us' : $options['cfw_button_title'];
  echo "<input id='plugin_text_string' name='plugin_options[cfw_button_title]' size='40' type='text' value='{$button_title}' />";
}
